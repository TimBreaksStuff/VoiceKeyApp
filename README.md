...
**Windows — [download VoiceKey 1.0.0 (win-x64)](https://github.com/TimBreaksStuff/VoiceKeyApp/releases/latest)**, unpack anywhere, run `VoiceKey.exe`. Self-contained: no .NET install needed. Windows will show a SmartScreen warning because the build is unsigned — *More info → Run anyway*.

macOS has no prebuilt download yet; build it from source below. Either way it is one command once the toolchain is there.
