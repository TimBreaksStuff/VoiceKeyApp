...
internal sealed class AppController : IDisposable
{
    ...
    private void HotKeyPressed()
    {
        ...
        switch (_status)
        {
            case DictationStatus.IdleState:
                try
                {
                    _recorder.Start(_preferences.StopOnSilence);
    ...
    private sealed class WindowActions(AppController controller) : IMainWindowActions
    {
        ...
        /// <summary>
        /// Takes effect on the next recording — the one in flight keeps the rule
        /// it started under, so a click cannot cut a sentence short.
        /// </summary>
        public void ToggleStopOnSilence()
        {
            controller._preferences = controller._preferences with
            {
                StopOnSilence = !controller._preferences.StopOnSilence,
            };
            controller._preferences.Save();
            controller.SyncUi();
        }
