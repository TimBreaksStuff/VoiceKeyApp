...
/// (what whisper wants), and auto-stops after ~1 s of trailing silence — but
...
internal sealed class AudioRecorder : IDisposable
{
    ...
    private const int SilenceSamplesNeeded = 16_000; // 1 s @ 16 kHz
    ...
    /// true the one time trailing silence should stop the recording.
    ...
    private bool Drain()
    {
        ...
        while ((read = _resampled.Read(_pullBuffer, 0, _pullBuffer.Length)) > 0)
        {
            ...
            if (_heardSpeech && _silentSampleCount >= SilenceSamplesNeeded && !_autoStopFired)
            {
                _autoStopFired = true;
                shouldStop = true;
            }
