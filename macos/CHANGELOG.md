...
## 1.4.1 — the same app, under the release the tag names

**Nothing here changed.** 1.4.1 is a Windows fix — the window's icon on a dark taskbar — and the Mac had nothing to fix. The version moves anyway because both platforms ship under one release tag, and the updater compares that tag to the version it is running: a 1.4.0 bundle downloaded from `v1.4.1` would offer the same update forever.
