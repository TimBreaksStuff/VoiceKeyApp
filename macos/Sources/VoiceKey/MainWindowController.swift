...
struct MainWindowModel: Equatable {
    ...
    var soundCues = true
    /// Whether a second of silence ends the recording.
    var stopOnSilence = true
...
final class MainWindowController: NSObject, NSWindowDelegate, NSTextFieldDelegate {
    ...
    private var stopOnSilenceLink: SidebarLink?
    ...
    private func makeSettingsCard() -> NSView {
        ...
        let silence = SidebarLink(title: RecordingStop.label(model.stopOnSilence)) { [weak self] in
            self?.actions?.windowToggleStopOnSilence()
        }
        stopOnSilenceLink = silence
        applyStopOnSilence(model.stopOnSilence)
        ...
        let stack = NSStackView(views: [heading, launch, sounds, silence, shortcut, rule,
    ...
    private func applyStopOnSilence(_ stopOnSilence: Bool) {
        stopOnSilenceLink?.title = RecordingStop.label(stopOnSilence)
