...
/// Taps the mic at its native format, converts to 16kHz mono Float32, and stops
/// itself when `RecordingStop` says the recording has ended — trailing silence,
/// or the cap.
final class AudioRecorder {
    ...
    private var stopOnSilence = true
    ...
    /// - Parameter stopOnSilence: The user's setting: false leaves the recording
    ///   running through any pause, so only a second press of the shortcut (or
    ///   the cap) ends it.
    func start(stopOnSilence: Bool) throws {
        lock.lock()
        samples.removeAll()
        heardSpeech = false
        silentSampleCount = 0
        ...
        input.installTap(onBus: 0, bufferSize: 4096, format: inFormat) { [weak self] buffer, _ in
            ...
            let shouldStop = !self.autoStopFired
                && RecordingStop.shouldStop(stopsOnSilence: self.stopOnSilence,
                                            heardSpeech: self.heardSpeech,
                                            silentSamples: self.silentSampleCount,
                                            totalSamples: self.samples.count)
